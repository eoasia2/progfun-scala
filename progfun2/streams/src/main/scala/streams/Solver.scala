...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] = neighbors.filter(x => !explored.contains(x._1))
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      val more = for {
        m <- initial
        next <- newNeighborsOnly(neighborsWithHistory(m._1, m._2), explored)
      } yield next
   ...
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from(((startBlock, Nil) :: Nil).to(LazyList), Set(startBlock))
