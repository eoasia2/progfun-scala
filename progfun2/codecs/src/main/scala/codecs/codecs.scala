...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] =
    Decoder.fromPartialFunction { case Json.Num(x) if x.isValidInt => x.toIntExact }
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] =
    Decoder.fromFunction {
      case json => for {
        name <- Decoder.field[String]("name").decode(json)
        age <- Decoder.field[Int]("age").decode(json)
